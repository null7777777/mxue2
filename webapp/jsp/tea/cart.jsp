...
<script type="text/javascript" src="js/tea/user_reg_login.js"></script>
<style type="text/css">
.wrapper {
	minheight: 400px;
}
								...
								<c:forEach items="${shopCart.map}" var="i">
									<tr class="row" id="pro-tr-${i.key}">
										<td><img class="img-responsive col-md-4"
											src="${i.value.tea.upLoadImg.imgSrc }" alt="" />
											<div class="col-md-8" style="padding:50px 0 50px 0;">
												<a href="teadetail?teaId=${i.key}">${i.value.tea.teaName}</a>
												<%-- <p>${i.value.tea.author}</p>
												<p>${i.value.tea.press}</p> --%>
											</div>
										</td>
										<td style="padding:50px 0 50px 0;">￥<i>${i.value.tea.price}</i></td>
										...
										<img class="icon" src="images/tea/close-icon.png" alt="" style="margin:50px 0 50px 0;width:15px"></a></td>
									</tr>
								</c:forEach>
								<tr class="row">
	  ...
	  //输入框处理
		...
		$("#tosettle").click(function(){
			$.get("UserServlet?action=landstatus",function(data){
				if(data.status=="y"){
					window.location.href="${basePath}jsp/tea/conorder.jsp";
				}else{
					$("#myModal").modal("show");
				}
			},"json")
